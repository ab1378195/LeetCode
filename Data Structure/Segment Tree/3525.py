...
class SegmentTree:
    ...
    # 合并信息
    def _merge_data(
        self, a: Tuple[int, List[int]], b: Tuple[int, List[int]]
    ) -> Tuple[int, List[int]]:
        cnt = a[1].copy()
        left_mul = a[0]
    ...
    def _maintain(self, node: int) -> None:
        self._tree[node] = self._merge_data(
            self._tree[node * 2], self._tree[node * 2 + 1]
        )
    ...
    def _query(
        self, node: int, l: int, r: int, ql: int, qr: int
    ) -> Tuple[int, List[int]]:
...
if __name__ == "__main__":
    ...
    print(solution.resultArray([1, 2, 4, 8, 16, 32], 4, [[0, 2, 0, 2], [0, 2, 0, 1]]))
    print(solution.resultArray([1, 1, 2, 1, 1], 2, [[2, 1, 0, 1]]))
